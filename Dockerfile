...
#     apt-get update && \
#     apt-get -t unstable install -y --no-install-recommends upx-ucl
...
# RUN if [ "$GOARCH" = "$BUILD_ARCH" ] && [ "$GOOS" = "$BUILD_OS" ]; then \
#         upx --best --lzma /tmp/optimux; \
#     else \
#         echo "Skipping compression since $BUILD_ARCH does not match $GOARCH or $BUILD_OS does not match $GOOS."; \
#     fi
...
COPY ./infra/dev.nginx.conf /etc/nginx/nginx.conf
# COPY ./infra/dev.resty.conf /etc/nginx/nginx.conf
