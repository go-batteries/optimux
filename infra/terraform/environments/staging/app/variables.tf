...
variable "ECS_SERVICE_MIN_CAPACITY" {
  type        = number
  default     = 1
  description = "Minimum number of ECS tasks for media-service-stage"
}

variable "ECS_SERVICE_MAX_CAPACITY" {
  type        = number
  default     = 10
  description = "Maximum number of ECS tasks for media-service-stage"
}

variable "ECS_SERVICE_DESIRED_COUNT" {
  type        = number
  default     = 2
  description = "Desired number of ECS tasks for media-service-stage"
}

variable "FFMPEG_HWACCEL" {
  type        = string
  default     = ""
  description = "FFmpeg hardware acceleration for staging (empty=none, vaapi=GPU)"
}

variable "FFMPEG_THREADS" {
  type        = number
  default     = 2
  description = "FFmpeg threads per command (capped at 2 for m5.2xlarge)"
}
